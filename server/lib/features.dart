...
List<double?> _atr(
    List<double?> h, List<double?> l, List<double?> c, int window) {
  final tr = List<double?>.filled(c.length, null);
  double? prevClose;
  for (var i = 0; i < c.length; i++) {
    final hi = h[i], lo = l[i], ci = c[i];
    if (hi == null || lo == null || ci == null) {
...
FeaturesResult computeFeatures(List<Bar> bars) {
  ...
  final d20 = List<double?>.generate(
      n,
          (i) => (close[i] != null && ema20[i] != null)
          ? ((close[i]! - ema20[i]!) / ema20[i]!)
          : null);
  final d50 = List<double?>.generate(
      n,
          (i) => (close[i] != null && ema50[i] != null)
          ? ((close[i]! - ema50[i]!) / ema50[i]!)
          : null);
  ...
  final atr14 = List<double?>.generate(
      n,
          (i) => (atr14Abs[i] != null && close[i] != null)
          ? (atr14Abs[i]! / close[i]!)
          : null);
  ...
  for (var i = 14; i < n; i++) {
    ...
    if (win.length == 15) {
      ...
      final s = sqrt(
          win.map((x) => pow(x - m, 2).toDouble()).reduce((a, b) => a + b) /
              win.length);
      vol15[i] = s;
    }
  }
  ...
  final dVwap = List<double?>.generate(
      n,
          (i) => (close[i] != null && vwap[i] != null)
          ? ((close[i]! - vwap[i]!) / vwap[i]!)
          : null);
  ...
  final volSurge = List<double?>.generate(
      n,
          (i) => (vol[i] != null && volEma20[i] != null && volEma20[i] != 0)
          ? (vol[i]! / volEma20[i]!)
          : null);
